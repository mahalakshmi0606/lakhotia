...
const AdminTaskPage = () => {
  ...
  const [statusCheckFilter, setStatusCheckFilter] = useState("All");
  ...
  useEffect(() => {
    ...
    // Apply status check filter
    if (statusCheckFilter !== "All") {
      if (statusCheckFilter === "Has Status") {
        filtered = filtered.filter(task => task.status_check && task.status_check.trim() !== "");
      } else if (statusCheckFilter === "No Status") {
        filtered = filtered.filter(task => !task.status_check || task.status_check.trim() === "");
      } else {
        filtered = filtered.filter(task => task.status_check === statusCheckFilter);
      }
    }
    ...
    // Apply search term - INCLUDES ALL FIELDS
    if (searchTerm.trim() !== "") {
      ...
      filtered = filtered.filter(task =>
        ...
        task.status_check?.toLowerCase().includes(searchLower) ||
    ...
    setCurrentPage(1);
  }, [tasks, searchTerm, statusFilter, priorityFilter, statusCheckFilter, dateFilter]);
  ...
  const taskStats = useMemo(() => {
    const stats = {
      ...
      withStatusCheck: tasks.filter(t => t.status_check && t.status_check.trim() !== "").length,
    ...
    <div className="p-4">
      ...
      <Row className="mb-4">
        ...
        <Col md={2} sm={6} className="mb-3">
          <Card className="text-center h-100 shadow-sm">
            <Card.Body className="p-3">
              <Card.Title className="text-secondary fs-4">{taskStats.withStatusCheck}</Card.Title>
              <Card.Text className="text-muted mb-0">With Status Check</Card.Text>
            </Card.Body>
          </Card>
        </Col>
      </Row>
        ...
        <Card.Body>
          <Row>
            ...
            <Col md={3} className="mb-3">
              <Form.Group controlId="searchTasks">
                ...
                <Form.Control
                  type="text"
                  placeholder="Search by PO number, item, brand, batch, quotation, company, etc..."
                  value={searchTerm}
            ...
            {/* Status Check Filter */}
            <Col md={2} className="mb-3">
              <Form.Group controlId="statusCheckFilter">
                <Form.Label className="small fw-bold">Status Check</Form.Label>
                <Form.Select
                  value={statusCheckFilter}
                  onChange={(e) => setStatusCheckFilter(e.target.value)}
                >
                  <option value="All">All</option>
                  <option value="Has Status">Has Status</option>
          ...
          <div className="d-flex justify-content-between align-items-center mt-2">
            ...
            <small className="text-muted">
              ...
              {statusCheckFilter !== "All" && <span className="me-2">Status Check: {statusCheckFilter}</span>}
      ...
      {loading ? (
        ...
        <Card className="text-center shadow-sm">
          <Card.Body className="py-5">
            ...
            <p className="text-muted">Tasks will appear here once they are created.</p>
          </Card.Body>
        </Card>
      ) : (
        <>
                ...
                currentTasks.map((task, index) => {
                  return (
                    ...
                    >
                      ...
                      <td>
                        <div>
                          <div><strong>{task.company_name || "-"}</strong></div>
                          {task.company_address && (
                            <small className="text-muted">{task.company_address}</small>
                          )}
                        </div>
                      </td>
                      <td>
                      ...
                      <td>
                        {task.status_check ? (
                          <Badge bg={
                            task.status_check === "Completed" ? "success" :
                            task.status_check === "Rework" ? "warning" : "secondary"
                          }>
                            {task.status_check}
                          </Badge>
                        ) : (
                ...
                <tr>
                  <td colSpan="17" className="text-center py-4">
                    <div className="text-muted">
                      ...
                      <p className="mt-2">
                        {searchTerm || statusFilter !== "All" || priorityFilter !== "All" || statusCheckFilter !== "All" || dateFilter !== "All" 
        ...
        <Modal.Body style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          {selectedViewTask && (
            <div className="row">
              ...
              {/* Quotation & Invoice Details Card */}
              <div className="col-md-6 mb-3">
                <Card className="h-100 shadow-sm">
                  <Card.Header className="bg-info text-white">
                    <h6 className="mb-0">Quotation & Invoice Details</h6>
                  </Card.Header>
                  <Card.Body>
                    <table className="table table-sm table-borderless mb-0">
                      <tbody>
