...
export default function QuotationModal() {
  ...
  const openTaskModal = (item, quotation) => {
    // Check if sales order item status is 'not_created' (item should be approved first)
    if (item.sales_order_item_status !== "not_created") {
      alert(`Item is already marked as "${item.sales_order_item_status}".`);
      return;
    }
    ...
    // Set task form data with ALL fields including brand code and batch number
    setTaskFormData({
      po_number: poNumber,
  ...
  const createTask = async () => {
    ...
    try {
      ...
      const response = await axios.post(API_TASKS, taskData);
      
      if (response.data) {
        alert("✅ Sales Order created successfully with PO number!");
        
        // Update item sales order status to "ordered" after task creation
        try {
          await axios.patch(
          ...
          await axios.patch(
            ...
            {
              remark: `Sales order created with PO: ${taskFormData.po_number}`,
              updated_by: loggedInUser
            }
          );
      ...
      console.error("❌ Error creating sales order:", err.response?.data || err.message);
      setTaskError(err.response?.data?.message || "Failed to create sales order. Please try again.");
      alert(`❌ Failed to create sales order: ${err.response?.data?.message || err.message}`);
    } finally {
    ...
    <Modal show={showTaskModal} onHide={() => {
      ...
      <Modal.Header closeButton className="bg-warning text-dark">
        <Modal.Title>
          ...
          Create Sales Order from Approved Item
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        {selectedItemForTask && (
          <>
            ...
            <Alert variant="warning">
              <i className="bi bi-exclamation-triangle me-2"></i>
              <strong>Note:</strong> Once created, the sales order will be assigned and the item's sales order status will be updated to "ordered".
              This will also mark the item as having a sales order created.
            </Alert>
          </>
        )}
      </Modal.Body>
    ...
    <Container fluid className="py-4">
      ...
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          ...
          <p className="text-muted mb-0">View and manage completed quotations - Create sales orders from approved items</p>
        </div>
      </div>
          ...
          {quotationForStatusUpdate && (
            <>
              <Alert variant="info">
                ...
                When you mark as "ordered", a sales order will be created.
              </Alert>
                      ...
                      <tr key={item.id || index}>
                        ...
                        <td>
                          ...
                          >
                            ...
                            <option value="ordered">🟢 Ordered (Create Sales Order)</option>
                        ...
                        <td>
                          {newStatus === "ordered" && currentStatus !== "ordered" && (
                            ...
                            >
                              <i className="bi bi-clipboard-check"></i> Create Sales Order
                            </Button>
                          )}
                          {currentStatus === "ordered" && (
                            <Badge bg="info">
                              <i className="bi bi-check-circle me-1"></i> Sales Order Created
                            </Badge>
                          )}
                        </td>
                      </tr>
                ...
                <tbody>
                  {(selectedQuotation.items || []).map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {(item.sales_order_item_status === "not_created" || !item.sales_order_item_status) && (
                          <Button
                            variant="warning"
                            size="sm"
                                ...
                                <div className="p-3 bg-light border-top">
                                  ...
                                  <div className="mt-3 d-flex justify-content-between align-items-center">
                                    <div className="text-muted small">
                                      Click on any item with "Not Created" status to create a sales order.
                                    </div>
                                    <Button
